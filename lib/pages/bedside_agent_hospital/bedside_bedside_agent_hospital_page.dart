...
class BedsideBedsideAgentHospitalPage extends StatelessWidget {
  ...
  BedsideBedsideAgentHospitalPage() {
    print("print");
  }

  BedsideBedsideAgentHospitalViewModel _bedsideBedsideAgentHospitalViewModel = BedsideBedsideAgentHospitalViewModel();
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          onDoubleTap: () {
            _bedsideBedsideAgentHospitalViewModel.scrollController.animateTo(-10.h, duration: Duration(milliseconds: 500), curve: Curves.linear);
          },
          child: Text("代理商-医院记录表",)
        ),
        ...
        children: [
          ProviderWidget(
            ...
            init: (BedsideBedsideAgentHospitalViewModel model) {
              model.initBedsideBedsideAgentHospitalList();
            },
            builder: (c,BedsideBedsideAgentHospitalViewModel model, _) {
              if(model.loading && model.bedsideBedsideAgentHospitalListModelDatas.isEmpty) {
                return Center(child: CupertinoActivityIndicator());
              }
              return ListView.builder(
                itemCount: model.bedsideBedsideAgentHospitalListModelDatas.length + 1,
                controller: model.scrollController,
                padding: EdgeInsets.only(bottom: 45.h),
                itemBuilder: (BuildContext context, int index){
                  if((model.totalCount == model.bedsideBedsideAgentHospitalListModelDatas.length || model.currPage == model.totalPage) && index == model.bedsideBedsideAgentHospitalListModelDatas.length) {
                    return Center(
                      child: Text("没有更多数据了",
                  ...
                  } else if(index == model.bedsideBedsideAgentHospitalListModelDatas.length) {
                    return Center(child: CupertinoActivityIndicator());
                  }
                  return Card(
                      ...
                      child: Column(
                        children: [
                          Row(
                            ...
                            children: [
                              ListCircleCheckbox(
                                check: model.ids.contains(model.bedsideBedsideAgentHospitalListModelDatas[index].id),
                                onChanged: (value) {
                                  if(value) {
                                    model.ids.add(model.bedsideBedsideAgentHospitalListModelDatas[index].id);
                                  }else {
                                    model.ids.remove(model.bedsideBedsideAgentHospitalListModelDatas[index].id);
                                  }
                                },
                              ),
                              SizedBox(width: 9.w,),
                              ListTableColumn(
                                dataList: [
                                  ListTableColumnModel(key: "代理商ID：",value: model.bedsideBedsideAgentHospitalListModelDatas[index].agentId.toString()),
                                  ListTableColumnModel(key: "医院ID：",value: model.bedsideBedsideAgentHospitalListModelDatas[index].hospitalId.toString()),
                                  ListTableColumnModel(key: "：",value: model.bedsideBedsideAgentHospitalListModelDatas[index].createdAt),
                                  ListTableColumnModel(key: "：",value: model.bedsideBedsideAgentHospitalListModelDatas[index].updatedAt),
                                ],
                              )
                            ],
                          ),
                            ...
                            child: Wrap(
                              ...
                              children: [
                                ListClickTextButton("修改",onPressed: () {
                                  Navigator.pushNamed(context,"${BedsideBedsideAgentHospitalPage.routeName.replaceFirst("Page", "SaveOrUpadtePage")}",arguments: SaveOrUpdateDto(
                                    index: index,
                                    titleStr: "修改",
                                    id: model.bedsideBedsideAgentHospitalListModelDatas[index].id,
                                    callback: (data) {
                                ...
                                ListClickTextButton("删除",onPressed: () {
                                  delHandler(context, model.bedsideBedsideAgentHospitalListModelDatas[index],index);
                                },),
                              ],
                            ),
                          )
                ...
                child: Row(
                  ...
                  children: [
                    ListCircleCheckbox(topPadding: 0.h,onChanged: (value) {
                      print(value);
                      _bedsideBedsideAgentHospitalViewModel.selectAll(value);
                    },),
                    SizedBox(width: 5.w,),
                    Text("全选",
                    ...
                    PopupMenuButton<int>(
                      onSelected: (value) {
                        if(value == 1) {
                          delSelectHandler(context, _bedsideBedsideAgentHospitalViewModel.ids);
                        }
                      },
                      itemBuilder: (context) {
  ...
  delHandler(BuildContext context,BedsideBedsideAgentHospitalListModelData data,int index) {
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            FlatButton(child: Text('确认'),onPressed: (){
                ...
                _bedsideBedsideAgentHospitalViewModel.bedsideBedsideAgentHospitalDelete(index, [data.id],success: (value) {
                  EasyLoading.dismiss();
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                FlatButton(child: Text('确认'),onPressed: (){
                    Navigator.pop(context);
                    EasyLoading.show(status: "删除中");
                    _bedsideBedsideAgentHospitalViewModel.bedsideBedsideAgentHospitalDeleteSelect(ids,success: (value) {
                      EasyLoading.dismiss();
                      EasyLoading.showToast("删除成功");
                      refreshList();
                    },error: (err) {
  ...
  refreshList() {
    ...
    _bedsideBedsideAgentHospitalViewModel.defaultData();
    _bedsideBedsideAgentHospitalViewModel.initBedsideBedsideAgentHospitalList(param: searchParam);
  }
}
