...
class BedsideBedsideTableErrorPage extends StatelessWidget {

  static const routeName = '/bedsidetableerrorPage';


  BedsideBedsideTableErrorPage() {
    print("print");
  }

  BedsideBedsideTableErrorViewModel _bedsideBedsideTableErrorViewModel = BedsideBedsideTableErrorViewModel();

  final TextEditingController searchController = TextEditingController();

  List<SearchModel> searchList = [
    SearchModel(key: 1,value: "设备编号",param: "deviceId"),
  ];
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          onDoubleTap: () {
            _bedsideBedsideTableErrorViewModel.scrollController.animateTo(-10.h, duration: Duration(milliseconds: 500), curve: Curves.linear);
          },
          child: Text("设备绑定错误记录表",)
        ),
        ...
        children: [
          ProviderWidget(
            ...
            init: (BedsideBedsideTableErrorViewModel model) {
              model.initBedsideBedsideTableErrorList();
            },
            builder: (c,BedsideBedsideTableErrorViewModel model, _) {
              if(model.loading && model.bedsideBedsideTableErrorListModelDatas.isEmpty) {
                return Center(child: CupertinoActivityIndicator());
              }
              return ListView.builder(
                itemCount: model.bedsideBedsideTableErrorListModelDatas.length + 1,
                controller: model.scrollController,
                padding: EdgeInsets.only(bottom: 45.h),
                itemBuilder: (BuildContext context, int index){
                  if((model.totalCount == model.bedsideBedsideTableErrorListModelDatas.length || model.currPage == model.totalPage) && index == model.bedsideBedsideTableErrorListModelDatas.length) {
                    return Center(
                      child: Text("没有更多数据了",
                  ...
                  } else if(index == model.bedsideBedsideTableErrorListModelDatas.length) {
                    return Center(child: CupertinoActivityIndicator());
                  }
                  return Card(
                      ...
                      child: Column(
                        children: [
                          Row(
                            ...
                            children: [
                              ListCircleCheckbox(
                                check: model.ids.contains(model.bedsideBedsideTableErrorListModelDatas[index].id),
                                onChanged: (value) {
                                  if(value) {
                                    model.ids.add(model.bedsideBedsideTableErrorListModelDatas[index].id);
                                  }else {
                                    model.ids.remove(model.bedsideBedsideTableErrorListModelDatas[index].id);
                                  }
                                },
                              ),
                              SizedBox(width: 9.w,),
                              ListTableColumn(
                                dataList: [
                                  ListTableColumnModel(key: "设备编号：",value: model.bedsideBedsideTableErrorListModelDatas[index].deviceId),
                                  ListTableColumnModel(key: "二维码编号：",value: model.bedsideBedsideTableErrorListModelDatas[index].number),
                                ],
                              )
                            ],
                          ),
  ...
  delHandler(BuildContext context,BedsideBedsideTableErrorListModelData data,int index) {
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            FlatButton(child: Text('确认'),onPressed: (){
                ...
                _bedsideBedsideTableErrorViewModel.bedsideBedsideTableErrorDelete(index, [data.id],success: (value) {
                  EasyLoading.dismiss();
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                FlatButton(child: Text('确认'),onPressed: (){
                    Navigator.pop(context);
                    EasyLoading.show(status: "删除中");
                    _bedsideBedsideTableErrorViewModel.bedsideBedsideTableErrorDeleteSelect(ids,success: (value) {
                      EasyLoading.dismiss();
                      EasyLoading.showToast("删除成功");
                      refreshList();
                    },error: (err) {
  ...
  refreshList() {
    ...
    _bedsideBedsideTableErrorViewModel.defaultData();
    _bedsideBedsideTableErrorViewModel.initBedsideBedsideTableErrorList(param: searchParam);
  }
}
