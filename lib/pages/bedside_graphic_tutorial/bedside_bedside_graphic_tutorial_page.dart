...
class BedsideBedsideGraphicTutorialPage extends StatelessWidget {

  static const routeName = '/bedsidegraphictutorialPage';


  BedsideBedsideGraphicTutorialPage() {
    print("print");
  }

  BedsideBedsideGraphicTutorialViewModel _bedsideBedsideGraphicTutorialViewModel = BedsideBedsideGraphicTutorialViewModel();
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          onDoubleTap: () {
            _bedsideBedsideGraphicTutorialViewModel.scrollController.animateTo(-10.h, duration: Duration(milliseconds: 500), curve: Curves.linear);
          },
          child: Text("图文教程",)
        ),
        ...
        children: [
          ProviderWidget(
            ...
            init: (BedsideBedsideGraphicTutorialViewModel model) {
              model.initBedsideBedsideGraphicTutorialList();
            },
            builder: (c,BedsideBedsideGraphicTutorialViewModel model, _) {
              if(model.loading && model.bedsideBedsideGraphicTutorialListModelDatas.isEmpty) {
                return Center(child: CupertinoActivityIndicator());
              }
              return ListView.builder(
                itemCount: model.bedsideBedsideGraphicTutorialListModelDatas.length + 1,
                controller: model.scrollController,
                padding: EdgeInsets.only(bottom: 45.h),
                itemBuilder: (BuildContext context, int index){
                  if((model.totalCount == model.bedsideBedsideGraphicTutorialListModelDatas.length || model.currPage == model.totalPage) && index == model.bedsideBedsideGraphicTutorialListModelDatas.length) {
                    return Center(
                      child: Text("没有更多数据了",
                  ...
                  } else if(index == model.bedsideBedsideGraphicTutorialListModelDatas.length) {
                    return Center(child: CupertinoActivityIndicator());
                  }
                  return Card(
                      ...
                      child: Column(
                        children: [
                          Row(
                            ...
                            children: [
                              ListCircleCheckbox(
                                check: model.ids.contains(model.bedsideBedsideGraphicTutorialListModelDatas[index].id),
                                onChanged: (value) {
                                  if(value) {
                                    model.ids.add(model.bedsideBedsideGraphicTutorialListModelDatas[index].id);
                                  }else {
                                    model.ids.remove(model.bedsideBedsideGraphicTutorialListModelDatas[index].id);
                                  }
                                },
                              ),
                              SizedBox(width: 9.w,),
                              ListTableColumn(
                                dataList: [
                                  ListTableColumnModel(key: "备注：",value: model.bedsideBedsideGraphicTutorialListModelDatas[index].remark),
                                  ListTableColumnModel(key: "创建时间：",value: model.bedsideBedsideGraphicTutorialListModelDatas[index].createdAt),
                                ],
                              )
                            ],
                          ),
                            ...
                            child: Wrap(
                              ...
                              children: [
                                ListClickTextButton("修改",onPressed: () {
                                  Navigator.pushNamed(context,"${BedsideBedsideGraphicTutorialPage.routeName.replaceFirst("Page", "SaveOrUpadtePage")}",arguments: SaveOrUpdateDto(
                                    index: index,
                                    titleStr: "修改",
                                    id: model.bedsideBedsideGraphicTutorialListModelDatas[index].id,
                                    callback: (data) {
                                ...
                                ListClickTextButton("删除",onPressed: () {
                                  delHandler(context, model.bedsideBedsideGraphicTutorialListModelDatas[index],index);
                                },),
                              ],
                            ),
                          )
                ...
                child: Row(
                  ...
                  children: [
                    ListCircleCheckbox(topPadding: 0.h,onChanged: (value) {
                      print(value);
                      _bedsideBedsideGraphicTutorialViewModel.selectAll(value);
                    },),
                    SizedBox(width: 5.w,),
                    Text("全选",
                    ...
                    PopupMenuButton<int>(
                      onSelected: (value) {
                        if(value == 1) {
                          delSelectHandler(context, _bedsideBedsideGraphicTutorialViewModel.ids);
                        }
                      },
                      itemBuilder: (context) {
  ...
  delHandler(BuildContext context,BedsideBedsideGraphicTutorialListModelData data,int index) {
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            FlatButton(child: Text('确认'),onPressed: (){
                ...
                _bedsideBedsideGraphicTutorialViewModel.bedsideBedsideGraphicTutorialDelete(index, [data.id],success: (value) {
                  EasyLoading.dismiss();
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                FlatButton(child: Text('确认'),onPressed: (){
                    Navigator.pop(context);
                    EasyLoading.show(status: "删除中");
                    _bedsideBedsideGraphicTutorialViewModel.bedsideBedsideGraphicTutorialDeleteSelect(ids,success: (value) {
                      EasyLoading.dismiss();
                      EasyLoading.showToast("删除成功");
                      refreshList();
                    },error: (err) {
  ...
  refreshList() {
    ...
    searchParam[searchList[currentSearchListIndex].getParam()] = searchController.text;
    _bedsideBedsideGraphicTutorialViewModel.defaultData();
    _bedsideBedsideGraphicTutorialViewModel.initBedsideBedsideGraphicTutorialList(param: searchParam);
  }
}
