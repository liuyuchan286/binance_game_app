...
class BedsideBedsideHospitalOfficePage extends StatelessWidget {

  static const routeName = '/bedsidehospitalofficePage';

  BedsideBedsideHospitalOfficeListDto dto;

  BedsideBedsideHospitalOfficePage(BedsideBedsideHospitalOfficeListDto dto){
    this.dto = dto;
    if(dto != null) {
      searchParam["hospitalId"] = dto.hospitalId;
    }
  }

  BedsideBedsideHospitalOfficeViewModel _bedsideBedsideHospitalOfficeViewModel = BedsideBedsideHospitalOfficeViewModel();

  final TextEditingController searchController = TextEditingController();

  List<SearchModel> searchList = [
    SearchModel(key: 1,value: "科室名称",param: "name"),
  ];
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          onDoubleTap: () {
            _bedsideBedsideHospitalOfficeViewModel.scrollController.animateTo(-10.h, duration: Duration(milliseconds: 500), curve: Curves.linear);
          },
          child: Text("医院科室记录表",)
        ),
        ...
        children: [
          ProviderWidget(
            ...
            builder: (c,BedsideBedsideHospitalOfficeViewModel model, _) {
              if(model.loading && model.bedsideBedsideHospitalOfficeListModelDatas.isEmpty) {
                return Center(child: CupertinoActivityIndicator());
              }
              return ListView.builder(
                itemCount: model.bedsideBedsideHospitalOfficeListModelDatas.length + 1,
                controller: model.scrollController,
                padding: EdgeInsets.only(bottom: 45.h),
                itemBuilder: (BuildContext context, int index){
                  if((model.totalCount == model.bedsideBedsideHospitalOfficeListModelDatas.length || model.currPage == model.totalPage) && index == model.bedsideBedsideHospitalOfficeListModelDatas.length) {
                    return Center(
                      child: Text("没有更多数据了",
                  ...
                  } else if(index == model.bedsideBedsideHospitalOfficeListModelDatas.length) {
                    return Center(child: CupertinoActivityIndicator());
                  }
                  return Card(
                      ...
                      child: Column(
                        children: [
                          Row(
                            ...
                            children: [
                              ListCircleCheckbox(
                                check: model.ids.contains(model.bedsideBedsideHospitalOfficeListModelDatas[index].id),
                                onChanged: (value) {
                                  if(value) {
                                    model.ids.add(model.bedsideBedsideHospitalOfficeListModelDatas[index].id);
                                  }else {
                                    model.ids.remove(model.bedsideBedsideHospitalOfficeListModelDatas[index].id);
                                  }
                                },
                              ),
                              SizedBox(width: 9.w,),
                              ListTableColumn(
                                dataList: [
                                  ListTableColumnModel(key: "科室名称：",value: model.bedsideBedsideHospitalOfficeListModelDatas[index].name,onTab: () {
                                    Navigator.of(context).pushNamed(BedsideBedsideHospitalRoomPage.routeName,arguments: BedsideBedsideHospitalRoomListDto(
                                      hospitalId: model.bedsideBedsideHospitalOfficeListModelDatas[index].hospitalId,
                                      officeId: model.bedsideBedsideHospitalOfficeListModelDatas[index].id,
                                    ));
                                  }),
                                  ListTableColumnModel(key: "创建时间",value: model.bedsideBedsideHospitalOfficeListModelDatas[index].createdAt),
                                ],
                              )
                            ],
                          ),
                            ...
                            child: Wrap(
                              ...
                              children: [
                                ListClickTextButton("修改",onPressed: () {
                                  Navigator.pushNamed(context,"${BedsideBedsideHospitalOfficePage.routeName.replaceFirst("Page", "SaveOrUpadtePage")}",arguments: SaveOrUpdateDto<BedsideBedsideHospitalOfficeListDto>(
                                    index: index,
                                    titleStr: "修改",
                                    id: model.bedsideBedsideHospitalOfficeListModelDatas[index].id,
                                    t: dto,
                                    callback: (data) {
                                ...
                                ListClickTextButton("删除",onPressed: () {
                                  delHandler(context, model.bedsideBedsideHospitalOfficeListModelDatas[index],index);
                                },),
                              ],
                            ),
                          )
                ...
                child: Row(
                  ...
                  children: [
                    ListCircleCheckbox(topPadding: 0.h,onChanged: (value) {
                      print(value);
                      _bedsideBedsideHospitalOfficeViewModel.selectAll(value);
                    },),
                    SizedBox(width: 5.w,),
                    Text("全选",
                    ...
                    PopupMenuButton<int>(
                      onSelected: (value) {
                        if(value == 1) {
                          delSelectHandler(context, _bedsideBedsideHospitalOfficeViewModel.ids);
                        }
                      },
                      itemBuilder: (context) {
  ...
  delHandler(BuildContext context,BedsideBedsideHospitalOfficeListModelData data,int index) {
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            FlatButton(child: Text('确认'),onPressed: (){
                ...
                _bedsideBedsideHospitalOfficeViewModel.bedsideBedsideHospitalOfficeDelete(index, [data.id],success: (value) {
                  EasyLoading.dismiss();
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                FlatButton(child: Text('确认'),onPressed: (){
                    Navigator.pop(context);
                    EasyLoading.show(status: "删除中");
                    _bedsideBedsideHospitalOfficeViewModel.bedsideBedsideHospitalOfficeDeleteSelect(ids,success: (value) {
                      EasyLoading.dismiss();
                      EasyLoading.showToast("删除成功");
                      refreshList();
                    },error: (err) {
  ...
  refreshList() {
    ...
    _bedsideBedsideHospitalOfficeViewModel.defaultData();
    _bedsideBedsideHospitalOfficeViewModel.initBedsideBedsideHospitalOfficeList(param: searchParam);
  }
}
