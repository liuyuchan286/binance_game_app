...
class BedsideBedsideConfigPage extends StatelessWidget {

  static const routeName = '/bedsideconfigPage';


  BedsideBedsideConfigPage() {
    print("print");
  }

  BedsideBedsideConfigViewModel _bedsideBedsideConfigViewModel = BedsideBedsideConfigViewModel();

  final TextEditingController searchController = TextEditingController();

  List<SearchModel> searchList = [
    SearchModel(key: 1,value: "配置名称",param: "key"),
  ];
    ...
    return Scaffold(
      appBar: AppBar(
        title: GestureDetector(
          onDoubleTap: () {
            _bedsideBedsideConfigViewModel.scrollController.animateTo(-10.h, duration: Duration(milliseconds: 500), curve: Curves.linear);
          },
          child: Text("系统配置表",)
        ),
        ...
        children: [
          ProviderWidget(
            ...
            init: (BedsideBedsideConfigViewModel model) {
              model.initBedsideBedsideConfigList();
            },
            builder: (c,BedsideBedsideConfigViewModel model, _) {
              if(model.loading && model.bedsideBedsideConfigListModelDatas.isEmpty) {
                return Center(child: CupertinoActivityIndicator());
              }
              return ListView.builder(
                itemCount: model.bedsideBedsideConfigListModelDatas.length + 1,
                controller: model.scrollController,
                padding: EdgeInsets.only(bottom: 45.h),
                itemBuilder: (BuildContext context, int index){
                  if((model.totalCount == model.bedsideBedsideConfigListModelDatas.length || model.currPage == model.totalPage) && index == model.bedsideBedsideConfigListModelDatas.length) {
                    return Center(
                      child: Text("没有更多数据了",
                  ...
                  } else if(index == model.bedsideBedsideConfigListModelDatas.length) {
                    return Center(child: CupertinoActivityIndicator());
                  }
                  return Card(
                      ...
                      child: Column(
                        children: [
                          Row(
                            ...
                            children: [
                              ListCircleCheckbox(
                                check: model.ids.contains(model.bedsideBedsideConfigListModelDatas[index].id),
                                onChanged: (value) {
                                  if(value) {
                                    model.ids.add(model.bedsideBedsideConfigListModelDatas[index].id);
                                  }else {
                                    model.ids.remove(model.bedsideBedsideConfigListModelDatas[index].id);
                                  }
                                },
                              ),
                              SizedBox(width: 9.w,),
                              ListTableColumn(
                                dataList: [
                                  ListTableColumnModel(key: "配置名称：",value: model.bedsideBedsideConfigListModelDatas[index].name),
                                  ListTableColumnModel(key: "配置值：",value: model.bedsideBedsideConfigListModelDatas[index].value),
                                  ListTableColumnModel(key: "说明（描述）：",value: model.bedsideBedsideConfigListModelDatas[index].desc),
                                ],
                              )
                            ],
                          ),
                            ...
                            child: Wrap(
                              ...
                              children: [
                                ListClickTextButton("修改",onPressed: () {
                                  Navigator.pushNamed(context,"${BedsideBedsideConfigPage.routeName.replaceFirst("Page", "SaveOrUpadtePage")}",arguments: SaveOrUpdateDto(
                                    index: index,
                                    titleStr: "修改",
                                    id: model.bedsideBedsideConfigListModelDatas[index].id,
                                    callback: (data) {
                                ...
                                ListClickTextButton("删除",onPressed: () {
                                  delHandler(context, model.bedsideBedsideConfigListModelDatas[index],index);
                                },),
                                ListClickTextButton("刷新",onPressed: () {
                                  refreshHandler(context, model.bedsideBedsideConfigListModelDatas[index],index);
                                },),
                              ],
                            ),
                          )
                ...
                child: Row(
                  ...
                  children: [
                    ListCircleCheckbox(topPadding: 0.h,onChanged: (value) {
                      print(value);
                      _bedsideBedsideConfigViewModel.selectAll(value);
                    },),
                    SizedBox(width: 5.w,),
                    Text("全选",
                    ...
                    PopupMenuButton<int>(
                      onSelected: (value) {
                        if(value == 1) {
                          delSelectHandler(context, _bedsideBedsideConfigViewModel.ids);
                        }
                      },
                      itemBuilder: (context) {
  ...
  delHandler(BuildContext context,BedsideBedsideConfigListModelData data,int index) {
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            FlatButton(child: Text('确认'),onPressed: (){
                ...
                _bedsideBedsideConfigViewModel.bedsideBedsideConfigDelete(index, [data.id],success: (value) {
                  EasyLoading.dismiss();
  ...
  refreshHandler(BuildContext context,BedsideBedsideConfigListModelData data,int index) {
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                FlatButton(child: Text('确认'),onPressed: (){
                  ...
                  _bedsideBedsideConfigViewModel.bedsideBedsideConfigRefresh(index, [data.id],success: (value) {
                    EasyLoading.dismiss();
            ...
            return AlertDialog(
              ...
              actions: <Widget>[
                ...
                FlatButton(child: Text('确认'),onPressed: (){
                    Navigator.pop(context);
                    EasyLoading.show(status: "删除中");
                    _bedsideBedsideConfigViewModel.bedsideBedsideConfigDeleteSelect(ids,success: (value) {
                      EasyLoading.dismiss();
                      EasyLoading.showToast("删除成功");
                      refreshList();
                    },error: (err) {
  ...
  refreshList() {
    ...
    searchParam[searchList[currentSearchListIndex].getParam()] = searchController.text;
    _bedsideBedsideConfigViewModel.defaultData();
    _bedsideBedsideConfigViewModel.initBedsideBedsideConfigList(param: searchParam);
  }
}
